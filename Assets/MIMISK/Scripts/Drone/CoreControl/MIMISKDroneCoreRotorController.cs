...
public class MIMISKDroneCoreRotorController : MonoBehaviour
{
    ...
    {
        if (!controllerEnabled || controlMode == ControlMode.Disabled)
        {
            return;
        }
        ...
        else if (controlMode == ControlMode.PathTracking)
        {
            ...
            EvaluatePathReference(
                missionTimerS,
                out referencePositionWorld,
                out referenceVelocityWorld,
                out referenceAccelerationWorld
            );

            if (missionTimerS >= GetPathDuration())
            {
                CapturePositionHold();
                lastEvent = "path_completed_position_hold";
            }
        }
        else if (controlMode == ControlMode.ExternalReference)
        {
            // External mission script owns the reference.
        }
    ...
    public void AutoFindReferences()
    {
        ...
        if (coreGamepadReceiver == null)
        {
            coreGamepadReceiver = GetComponent<MIMISKDroneCoreGamepadReceiver>();
        }
    ...
    private void ForceUdpReceiverRawInputOnly()
    {
        if (udpReceiver == null)
        {
            return;
        ...
        Type receiverType = udpReceiver.GetType();

        FieldInfo suppressField =
            receiverType.GetField(
        ...
        {
            suppressField.SetValue(udpReceiver, true);
        }

        FieldInfo allowButtonsField =
            receiverType.GetField(
                "allowModeButtonsWhileSuppressed",
                BindingFlags.Instance | BindingFlags.Public
            );

        if (allowButtonsField != null)
        {
            allowButtonsField.SetValue(udpReceiver, true);
        }
    }
    ...
    private void CacheAndDisableLegacyIfNeeded()
    {
        ...
        if (keepUdpReceiverEnabled && udpReceiver != null)
        {
            ...
            // The UDP receiver stays alive for raw gamepad data only.
            // It must not send movement commands to any old model controller.
            ForceUdpReceiverRawInputOnly();
        }
    }

    public void InitializeMotorsAtHover()
    {
        float m = useRigidbodyMass && rb != null ? rb.mass : massKg;

        float hoverPerRotorN =
            Mathf.Clamp(m * gravity / 4.0f, 0.0f, maxThrustPerRotorN);
    ...
    public void CutMotors()
    {
        ...
        totalThrustCommandN = 0.0f;
        torqueCommandBodyNm = Vector3.zero;

        commandedAccelerationWorld = Vector3.zero;
        attitudeErrorBodyRad = Vector3.zero;
        angularVelocityBodyRadS = Vector3.zero;

        lastEvent = "motors_cut";
    }

    public void SetMotorsToHoverNow()
    {
        float m = useRigidbodyMass && rb != null ? rb.mass : massKg;
        ...
        motorThrustActualN = motorThrustCommandN;
        lastEvent = "motors_hover_initialized";
    }
    ...
    private void ComputeOuterPathPid(float dt)
    {
        ...
        integralErrorWorld.x =
            Mathf.Clamp(integralErrorWorld.x, -integralLimitXYZ.x, integralLimitXYZ.x);

        integralErrorWorld.y =
            Mathf.Clamp(integralErrorWorld.y, -integralLimitXYZ.y, integralLimitXYZ.y);

        integralErrorWorld.z =
            Mathf.Clamp(integralErrorWorld.z, -integralLimitXYZ.z, integralLimitXYZ.z);

        commandedAccelerationWorld = new Vector3(
            referenceAccelerationWorld.x
                + kpXZ * positionErrorWorld.x
                + kdXZ * velocityErrorWorld.x
                + kiXZ * integralErrorWorld.x,

            referenceAccelerationWorld.y
                + kpY * positionErrorWorld.y
                + kdY * velocityErrorWorld.y
                + kiY * integralErrorWorld.y,

            referenceAccelerationWorld.z
                + kpXZ * positionErrorWorld.z
                + kdXZ * velocityErrorWorld.z
                + kiXZ * integralErrorWorld.z
        );

        float maxHorizontalAccel =
            gravity * Mathf.Tan(maxTiltDeg * Mathf.Deg2Rad);

        Vector2 horizontalAccel =
            new Vector2(
        ...
        {
            horizontalAccel =
                horizontalAccel.normalized * maxHorizontalAccel;
    ...
    private void ComputeAttitudeTorque()
    {
        ...
        torqueCommandBodyNm = new Vector3(
            attitudeKpNmPerRad.x * attitudeErrorBodyRad.x
                - rateKdNmPerRadS.x * angularVelocityBodyRadS.x,

            attitudeKpNmPerRad.y * attitudeErrorBodyRad.y
                - rateKdNmPerRadS.y * angularVelocityBodyRadS.y,

            attitudeKpNmPerRad.z * attitudeErrorBodyRad.z
                - rateKdNmPerRadS.z * angularVelocityBodyRadS.z
        );
    ...
    private void BuildAllocationMatrix()
    {
        ...
        {
            new Vector3(-armX_M, 0.0f, -armZ_M), // FL
            new Vector3( armX_M, 0.0f, -armZ_M), // FR
            new Vector3(-armX_M, 0.0f,  armZ_M), // RL
            new Vector3( armX_M, 0.0f,  armZ_M)  // RR
        };
    ...
    private void AllocateAndApplyRotorForces(float dt)
    {
        ...
        motorThrustCommandN.x =
            Mathf.Clamp(motorThrustCommandN.x, 0.0f, maxThrustPerRotorN);

        motorThrustCommandN.y =
            Mathf.Clamp(motorThrustCommandN.y, 0.0f, maxThrustPerRotorN);

        motorThrustCommandN.z =
            Mathf.Clamp(motorThrustCommandN.z, 0.0f, maxThrustPerRotorN);

        motorThrustCommandN.w =
            Mathf.Clamp(motorThrustCommandN.w, 0.0f, maxThrustPerRotorN);
    ...
    private void OpenLog()
    {
        ...
        Debug.Log("[MIMISK] Core rotor log: " + currentLogPath);
    }
    ...
    private void WriteLogIfDue(float dt)
    {
        ...
        Vector3 trueP =
            rb != null ? rb.position : Vector3.zero;

        Vector3 trueV =
            rb != null ? rb.linearVelocity : Vector3.zero;

        float trueYaw =
            rb != null ? rb.rotation.eulerAngles.y : 0.0f;
