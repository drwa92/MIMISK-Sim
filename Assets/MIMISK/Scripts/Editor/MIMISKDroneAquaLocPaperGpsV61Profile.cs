...
public static class MIMISKDroneAquaLocPaperGpsV61Profile
{
    [MenuItem("MIMISK/Drone/Localization/Apply Profile/Paper-Equivalent GPS v6.2 Origin-Sync")]
    public static void ApplyPaperGpsV62()
    {
        ...
        if (gnss != null)
        {
            ...
            // The reference paper reports GPS horizontal accuracy = 0.50 m,
            // vertical accuracy = 1.00 m, and velocity accuracy = 0.10 m/s.
        ...
        // v6.2 fix: true GNSS-origin averaging.
        // Use enough time and enough fresh GNSS samples before enabling fusion.
        ...
        // GNSS prefilter and altitude prefilter.
        ...
        // Position/velocity Kalman prediction.
        ...
        // Measurement sigmas matched to paper-equivalent GPS.
        ...
        // Output smoothing and safe lead compensation.
        // The previous 0.75 s lead was too aggressive. v6.2 uses origin sync first,
        // then a small lead only to compensate residual filtering lag.
        SetBool(so, "enableOutputSmoothing", true);
        SetFloat(so, "outputPositionResponseHz", 8.5f);
        SetFloat(so, "outputVelocityResponseHz", 11.0f);
        SetFloat(so, "outputYawResponseHz", 10.0f);

        SetBool(so, "enableOutputLeadCompensation", true);
        SetFloat(so, "horizontalOutputLeadSeconds", 0.12f);
        SetFloat(so, "verticalOutputLeadSeconds", 0.02f);
        SetFloat(so, "maxOutputLeadCorrectionM", 0.25f);
        ...
        Debug.Log("[MIMISK] Applied AquaLoc Paper-Equivalent GPS v6.2 Origin-Sync profile.");
    }
    ...
    private static void SetFloat(SerializedObject so, string name, float value)
    {
        ...
        {
            Debug.LogWarning("[AquaLoc PaperGPS v6.2] Missing float: " + name);
        }
    }
    ...
    private static void SetInt(SerializedObject so, string name, int value)
    {
        ...
        {
            Debug.LogWarning("[AquaLoc PaperGPS v6.2] Missing int: " + name);
        }
    }
    ...
    private static void SetBool(SerializedObject so, string name, bool value)
    {
        ...
        {
            Debug.LogWarning("[AquaLoc PaperGPS v6.2] Missing bool: " + name);
        }
    }
    ...
    private static void SetObject(SerializedObject so, string name, UnityEngine.Object value)
    {
        ...
        {
            Debug.LogWarning("[AquaLoc PaperGPS v6.2] Missing object: " + name);
        }
    }
}
